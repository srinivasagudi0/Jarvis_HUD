...
def respond_to_command(command, weather, system_name, history=None):
    ...
    prompt = (
        ...
        f"{_conversation_text(history or [])}\n"
        f"User: {command}"
    )
    try:
        ...
        return text or _read_response_output(response)
...
def _read_response_output(response):
